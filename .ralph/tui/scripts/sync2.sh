...
# SCRIPT_DIR is where this script lives (package's scripts/ directory)
export SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# PROJECT_DIR is where the user's data lives (passed from ralph.sh or env var, defaults to cwd)
# This is where .ralph/ contains orchestrate.md, workflows/, logs, etc.
PROJECT_DIR="${RALPH_PROJECT_DIR:-$(pwd)}"
export PROJECT_DIR

# User data paths (in user's project)
RALPH_DATA_DIR="$PROJECT_DIR/.ralph"
export ORCHESTRATE_PROMPT="$RALPH_DATA_DIR/orchestrate.md"
export ASSIGNMENT_FILE="$RALPH_DATA_DIR/planning/assignment.json"
export LOG_FILE="$RALPH_DATA_DIR/claude_output.jsonl"
...
mkdir -p "$RALPH_DATA_DIR/planning"
