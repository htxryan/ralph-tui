...
- **task_id**: The task identifier (required) - corresponds to `.ai-docs/plans/<task_id>/`
...
- Input:  .ai-docs/plans/<task-id>/task.md (required - defines the task)
- Input:  .ai-docs/plans/<task-id>/research.md (optional - from research_codebase)
- Output: .ai-docs/plans/<task-id>/plan.md (generated by this command)
```
...
1. **Validate the task file exists**:
   - Check for `.ai-docs/plans/<task_id>/task.md`
   - If it does NOT exist, respond:
     ```
     Error: Task file not found.

     Expected: .ai-docs/plans/<task_id>/task.md
...
5. **If no research exists**, respond with:
   ```
   I'll help you create a detailed implementation plan for task: <task_id>

   Task file: .ai-docs/plans/<task_id>/task.md
   Output: .ai-docs/plans/<task_id>/plan.md

   Based on the task file, I understand we need to:
   [Summary of what task.md describes]

   Note: No existing research found at .ai-docs/plans/<task_id>/research.md
   Consider running /refine/research_codebase <task_id> first for thorough research.

   Would you like me to proceed with planning, or run research first?
   ```

   Then wait for the user's input.

6. **If research exists**, respond with:
   ```
...
1. **Read all mentioned files immediately and FULLY**:
   - Task document: `.ai-docs/plans/<task_id>/task.md` (REQUIRED - already read)
   - Research document: `.ai-docs/plans/<task_id>/research.md` (if exists)
...
5. **Present informed understanding and focused questions**:
   ```
   Based on task.md and my research of the codebase, I understand we need to [accurate summary].
...
1. **Write the plan** to `.ai-docs/plans/<task_id>/plan.md`
...
- Task definition: `.ai-docs/plans/<task_id>/task.md`
- Research doc: `.ai-docs/plans/<task_id>/research.md`
...
Task file: .ai-docs/plans/my-feature/task.md
Research: .ai-docs/plans/my-feature/research.md
Output: .ai-docs/plans/my-feature/plan.md
